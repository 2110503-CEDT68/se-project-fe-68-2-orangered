...
export default function AnnouncementManager() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-10 text-text-main animate-in fade-in duration-700">
      <div className="mb-12">
        <h1 className="text-3xl font-serif tracking-tight mb-2 text-text-main">
      ...
      <form
        ...
        className="bg-card border border-card-border p-8 rounded-2xl mb-16 shadow-2xl backdrop-blur-md relative overflow-hidden transition-all duration-500 hover:border-accent/30"
      >
        ...
        <div className="space-y-6">
          <div className="group">
            <p className="text-[8px] uppercase tracking-[0.2em] text-text-sub mb-2 group-focus-within:text-accent transition-colors">
              Heading
            </p>
            <input
              type="text"
        ...
        <div className="flex gap-4 mt-8">
          <button
            type="submit"
            className="px-8 py-2.5 bg-accent/20 border border-accent/50 text-accent text-[10px] uppercase tracking-[0.4em] hover:bg-accent hover:text-white transition-all rounded-sm font-bold"
          >
          ...
          {editingId && (
            <button
              ...
              className="px-8 py-2.5 border border-card-border text-text-sub text-[10px] uppercase tracking-[0.4em] hover:text-text-main transition-all rounded-sm"
            >
              Discard
            </button>
          )}
      ...
      <div className="space-y-6">
        <p className="text-[9px] uppercase tracking-[0.5em] text-text-sub mb-8 text-center">
          — Active Registry Entries —
        </p>
        {announcements.map((ann) => (
          <div
            key={ann._id}
            className="group flex flex-col md:flex-row justify-between items-start md:items-center bg-card/40 p-6 rounded-xl border border-card-border transition-all duration-500 hover:bg-card hover:translate-x-1 gap-6"
          >
            {/* แสดงรูปภาพถ้ามี */}
            <div className="flex gap-6 items-center flex-1">
              ...
              <div className="space-y-1">
                <p className="font-serif text-lg text-text-main group-hover:text-accent transition-colors">
                  {ann.title}
                </p>
