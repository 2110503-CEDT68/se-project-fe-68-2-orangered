...
export default function ReservationForm({ shop }: { shop: ShopItem }) {
  ...
  const shopOpen = date
    ? date
        .hour(parseInt(shop.openClose.open.split(":")[0]))
        .minute(parseInt(shop.openClose.open.split(":")[1]))
        .second(0)
    : dayjs(shop.openClose.open, "HH:mm");

  const shopClose = date
    ? date
        .hour(parseInt(shop.openClose.close.split(":")[0]))
        .minute(parseInt(shop.openClose.close.split(":")[1]))
        .second(0)
  ...
  const getValidPromo = (promotions?: any[], targetDate?: Dayjs | null) => {
    if (!promotions || promotions.length === 0) return null;

    const referenceDate = targetDate ? targetDate.toDate() : new Date();
  ...
  async function handleCreateReservation() {
    ...
    const activePromo = getValidPromo(selectedTreatment.promotions, date);
    ...
    if (
      selectedDateTime.isBefore(actualMinTime) ||
      selectedDateTime.isAfter(shopClose)
    ) {
      setValidationError(
    ...
    try {
      ...
      setValidationError(err.message || "A disturbance in the connection.");
    }
  }
            ...
            <FormControl fullWidth size="small">
              ...
              >
                ...
                {shop.massageType
                  ...
                  .map((type) => {
                    const activePromo = getValidPromo(type.promotions, date);
                      ...
                      >
                        <div className="flex justify-between w-full text-[10px] uppercase tracking-[0.15em] font-medium items-center">
                          <div className="flex flex-col">
                            ...
                            {hasDiscount && (
                              <span className="text-[8px] text-accent font-bold italic tracking-tight animate-pulse">
                                ✦ {activePromo.title}
                              </span>
                            )}
                          </div>
